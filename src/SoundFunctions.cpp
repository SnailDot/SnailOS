...
#include "MenuStates.h"  // Add this include
#include "DisplayTools.h"  // Add this for drawSettingsMenu

// Add extern declaration for currentState
...
const char* soundSettingsItems[] = {
    "Volume Control",
    "Test Sound"
};

const int NUM_SOUND_SETTINGS_ITEMS = 2;
const int SOUND_SETTINGS_VISIBLE_ITEMS = 2;

// Sound settings state variables
int soundSettingsViewportStart = 0;
int selectedSoundSetting = 0;
...
void drawSoundSettingsMenu() {
    ...
    M5.Display.println("Sound Settings");
    M5.Display.drawLine(10, 40, M5.Display.width() - 10, 40, TFT_BLACK);
    
    // Draw menu items
    int y = 60;
    for (int i = 0; i < SOUND_SETTINGS_VISIBLE_ITEMS; i++) {
        int itemIndex = soundSettingsViewportStart + i;
        if (itemIndex >= NUM_SOUND_SETTINGS_ITEMS) break;
        
        // Draw button background
        if (itemIndex == selectedSoundSetting) {
        ...
        M5.Display.println(soundSettingsItems[itemIndex]);
        y += 50;
    }
}

void updateSoundSettingsViewport() {
    // Ensure selected item is visible
    if (selectedSoundSetting < soundSettingsViewportStart) {
        soundSettingsViewportStart = selectedSoundSetting;
    } else if (selectedSoundSetting >= soundSettingsViewportStart + SOUND_SETTINGS_VISIBLE_ITEMS) {
        soundSettingsViewportStart = selectedSoundSetting - SOUND_SETTINGS_VISIBLE_ITEMS + 1;
    }
}
...
void drawVolumeControl() {
    ...
    M5.Display.setTextColor(TFT_BLACK);
    M5.Display.setCursor(10, 10);
    M5.Display.println("Volume Control");
    M5.Display.drawLine(10, 40, M5.Display.width() - 10, 40, TFT_BLACK);
    ...
    // Draw volume bar
    int barWidth = M5.Display.width() - 40;
    int barHeight = 30;
    int barX = 20;
    int barY = 80;
    
    // Draw background
    M5.Display.fillRect(barX, barY, barWidth, barHeight, TFT_DARKGREEN);
    
    // Draw volume level
    int volumeWidth = (barWidth * currentVolume) / 255;
    M5.Display.fillRect(barX, barY, volumeWidth, barHeight, TFT_BLACK);
    
    // Draw instructions
    M5.Display.setTextSize(1);
    M5.Display.setCursor(20, barY + barHeight + 20);
    M5.Display.println("Use Up/Down to adjust volume");
    M5.Display.setCursor(20, barY + barHeight + 40);
    M5.Display.println("Enter to test sound");
    M5.Display.setCursor(20, barY + barHeight + 60);
    M5.Display.println("ESC to return");
} 